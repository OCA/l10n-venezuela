...
class SeniatUnidadTributaria(models.Model):
    ...
    amount = fields.Float("Valor de la Unidad", required=True,)
    ref = fields.Text("Referencia de la gaceta", required=True,)
...
class SeniatTipoPersonaISLR(models.Model):
    ...
    name = fields.Char("Nombre Persona", required=True,)
    code = fields.Char("Abreviado", required=True,)
...
class SeniatFactor(models.Model):
    ...
    date = fields.Date("Fecha", required=True,)
    amount = fields.Float("Valor del factor", required=True,)
...
class BandaCaculo(models.Model):
    ...
    withholding_amount = fields.Float("Monto fijo de retención", help="Monto Retención")
...
class SeniatTablaIslr(models.Model):
    ...
    factor_id = fields.Many2one("seniat.factor", "Factor Calculo",)
    ...
    complete_name = fields.Char("Código SENIAT", compute="_compute_complete_name",)
    seniat_ut_id = fields.Many2one("seniat.ut", "Valor Unidad Tributaria")
    code_identification_seniat = fields.Char(
        string="Código Identificación Seniat", help="Código Identificación Seniat",
    )
