...
class L10nVeEdocProviderHka(models.AbstractModel):
    ...
    def _hka_base_url(self, company):
        ...
        if not url:
            raise UserError(
                self.env._(
                    "Configure The Factory HKA's production URL in "
                    "%(company)s's Accounting Settings (the provider hands it "
                    "over with the contract).",
    ...
    def _hka_auth(self, company):
        if not company.l10n_ve_edoc_user or not company.l10n_ve_edoc_password:
            raise UserError(
                self.env._(
                    "Configure The Factory HKA's username and password in "
                    "%(company)s's Accounting Settings.",
                    company=company.display_name,
                )
            )
        response = requests.post(
            ...
            json={
                "usuario": company.l10n_ve_edoc_user,
        ...
        if not data.get("token"):
            raise UserError(
                self.env._(
                    "The Factory HKA returned no token: %(message)s",
                    message=data.get("mensaje") or self.env._("no message"),
                )
            )
        return data
    ...
    def _hka_token(self, company, force=False):
        ...
        key = f"l10n_ve_digital_billing.hka_token_{company.id}"
        if not force:
            ...
            if cached:
                try:
                    ...
                    # An unreadable cache entry is not an error worth raising:
                    # authenticating again below replaces it.
                    _logger.info(
                        "Discarding unreadable HKA token cache for company %s",
                        company.id,
                    )
        data = self._hka_auth(company)
        icp.set_param(
    ...
    def _hka_post(self, company, path, payload):
        for attempt in (1, 2):
            ...
            response = requests.post(
                self._hka_base_url(company) + path,
                json=payload,
    ...
    def _hka_parse(self, response):
        try:
            ...
            raise UserError(
                self.env._(
                    "The Factory HKA responded HTTP %(status)s without JSON: %(body)s",
        ...
        if code != "200":
            ...
            raise UserError(
                self.env._(
                    "The Factory HKA [%(code)s]: %(message)s%(details)s",
                    code=code,
                    message=data.get("mensaje") or self.env._("Provider error"),
    ...
    def _hka_payload(self, move, vals):
        ...
        identification = {
            ...
            "tipoDeVenta": (
                "Contado" if vals.get("sale_type") == "cash" else "Crédito"
            ),
            # Idempotency: identifies this emission attempt to HKA.
            "transaccionId": f"ODOO-{company.id}-{move.id}",
        }
        ...
        details = [
            {
                ...
                "descuentoMonto": self._hka_amount(line.get("discount_amount", 0.0)),
        ...
        totals = {
            ...
            "subtotal": self._hka_amount(
                vals.get("untaxed_total", vals["taxed_total"] + vals["exempt_total"])
            ),
        ...
        return {
            "documentoElectronico": self._hka_clean(
                {
                    "encabezado": {
    ...
    def _hka_parse_expiry(value):
        ...
        if match:
            ...
            return fields.Datetime.from_string(f"{day} {clock}")
