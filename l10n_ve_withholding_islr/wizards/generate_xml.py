...
class GenerateXmlWhIslr(models.TransientModel):
    ...
    def action_generate_xml(self):
        ...
        if not vouchers:
            raise UserError(_("No confirmed ISLR Withholding vouchers found for the selected date range."))
        ...
        for v in vouchers:
            ...
            for l in v.line_ids:
                ...
                ET.SubElement(det, "FechaOperacion").text = (inv.invoice_date or v.date).strftime("%d/%m/%Y")
                ET.SubElement(det, "CodigoConcepto").text = l.concept_id.code or "001"
                ET.SubElement(det, "MontoOperacion").text = f"{l.base_amount:.2f}"
                ET.SubElement(det, "PorcentajeRetencion").text = f"{l.wh_percentage:.2f}"
