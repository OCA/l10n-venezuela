...
class AccountVatLedgerXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, account_vat):
        for obj in account_vat:
            ...
            date_line = workbook.add_format({"border": 1, "num_format": "dd-mm-yyyy"})
            ...
            sheet.write(4, 6, "Número Nota de Crédito", bold)
            sheet.write(4, 7, "Número Nota de Débito", bold)
            ...
            for invoice in reversed(obj.invoice_ids):
                if obj.type == "purchase":
                    ...
                    sheet.write(
                        row, 3, invoice.l10n_ve_document_number or "FALSE", line
                    )
                    ...
                    if invoice.amount_tax_signed == 0:
                        ...
                        sheet.write(row, 12, str(invoice.amount_tax_signed * -1), line)
                        sheet.write(row, 13, 0, line)
                        sheet.write(
                            row, 14, str(invoice.amount_total_signed * -1), line
                        )
                        ...
                        sheet.write(
                            ...
                            int(
                                (
                                    str(invoice.amount_by_group[0][0]).replace(
                                        "IVA ", ""
                                    )
                                ).replace("%", "")
                            ),
                        ...
                        sheet.write(
                            row, 11, str(invoice.amount_untaxed_signed * -1), line
                        )
                        sheet.write(row, 12, str(invoice.amount_tax_signed * -1), line)
                        sheet.write(
                            row, 13, str(invoice.amount_total_signed * -1), line
                        )
