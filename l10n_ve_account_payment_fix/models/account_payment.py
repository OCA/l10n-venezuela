...
class AccountPayment(models.Model):
    ...
    # campo a ser extendido y mostrar un nombre determinado en las lineas de
    # pago de un payment group o donde se desee (por ej. con cheque, retención,
    # etc)
    payment_method_description = fields.Char(
        compute="_compute_payment_method_description", string="Payment Method Desc.",
    )
    ...
    # nuevo campo función para definir dominio de los métodos
    ...
    journal_ids = fields.Many2many(
        "account.journal", compute="_compute_journals", string="Journals",
    )
    ...
    def _onchange_currency(self):
        """Anulamos método nativo que pisa el monto remanente que pasamos
    ...
    def _onchange_payment_type(self):
        """
        Sobre escribimos y desactivamos la parte del dominio de la función
        ...
        if not self.invoice_ids:
            ...
            # limpiamos journal ya que podría no estar disponible para la nueva
            # operación y ademas para que se limpien los payment methods
    ...
    def _onchange_partner_type(self):
        """
        Agregamos dominio en vista ya que se pierde si se vuelve a entrar
        Anulamos función original porque no haría falta
        """
        return False

    def _onchange_amount(self):
        """
        Anulamos este evento onchange que termina cambiando el domain de journals
        y no es compatible con multicia y se pierde al guardar.
        TODO: ver que Odoo con este onchange llama a
        _compute_journal_domain_and_types quien devolvería un journal genérico
    ...
    def _onchange_journal(self):
        ...
        if self.journal_id:
            ...
            # si se eligió de origen el mismo diario de destino, lo reiniciamos
