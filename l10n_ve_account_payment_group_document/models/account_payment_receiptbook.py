...
class AccountPaymentReceiptbook(models.Model):
    ...
    sequence = fields.Integer(
        "Sequence", help="Used to order the receiptbooks", default=10,
    )
    name = fields.Char("Name", size=64, required=True, index=True,)
    partner_type = fields.Selection(
        [("customer", "Customer"), ("supplier", "Vendor")], required=True, index=True,
    )
    next_number = fields.Integer(
        related="sequence_id.number_next_actual", readonly=False,
    )
    ...
    def create(self, vals):
        sequence_type = vals.get(
            "sequence_type", self._context.get("default_sequence_type", False)
        )
        ...
        if (
            ...
            and not vals.get("sequence_id", False)
            and company_id
        ):
            seq_vals = {
                "name": vals["name"],
