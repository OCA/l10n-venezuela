...
class AccountPaymentGroupInvoiceWizard(models.TransientModel):
    ...
    use_documents = fields.Boolean(
        related="journal_id.l10n_latam_use_documents", string="Use Documents?",
    )
    journal_document_type_id = fields.Many2one(
        "l10n_latam.document.type", "Document Type", ondelete="cascade",
    )
    document_sequence_id = fields.Many2one(
        "ir.sequence", compute="_compute_l10n_latam_sequence"
    )
    ...
    def get_invoice_vals(self):
        invoice_vals = super(AccountPaymentGroupInvoiceWizard, self).get_invoice_vals()
        invoice_vals.update(
            {
