...
class JournalReportCustomHandler(models.AbstractModel):
    ...
    def _report_custom_engine_journal_report(
        ...
        account_alias = query.join(
            ...
            rhs_column="id",
            # Custom link name to avoid potential alias clash with what is generated by
            # _field_to_sql for the groupby below
            link="account_id_for_code",
        )
    ...
    def _custom_line_postprocessor(self, report, options, lines):
        """
        Process the lines generated by the engine to add metadata and add the tax
        summary lines
        """
        new_lines = []
                ...
                new_lines.append(
                    {
                        ...
                        "unfoldable": False,
                        "colspan": len(options["columns"]) + 1,
                        # We want it to take the whole line. It makes it easier to
                        # unfold it.
                    }
                )
                summary_line = {
    ...
    def export_to_xlsx(self, options, response=None):  # noqa: C901
        """
        ...
        # We need to use fonts to calculate column width otherwise column width would be
        # ugly
        # Using Lato as reference font is a hack and is not recommended. Customer
        # computers don't have this font by default and so
        # the generated xlsx wouldn't have this font. Since it is not by default, we
        # preferred using Arial font as default and keep
                ...
                for column in columns:
                    ...
                    if line.get(column["label"], {}).get("data"):
                        ...
                        if (
                            ...
                            # if the cell has bold styling, should only be on the first
                            # line of each aml
    ...
    def _write_cell(
        ...
        :param fonts:           The fonts used to calculate the size of each cells. We
        use Lato because we cannot get Arial but, we write in Arial since we cannot
        embed Lato on the worksheet
        ...
        :param bg_color:        The background color of the cell in hex or string ex:
        '#fff' default: 'white'
        :param align:           The alignement of the text ex: 'left', 'right', 'center'
        default: 'left'
        :param border_bottom:   The width of the bottom border default: 0
        :param border_top:      The width of the top border default: 0
        :param border_color:    The color of the borders in hex or string default:
        '0x000'
        """
        ...
        if colspan == 1:
            if datetime:
                ...
                # Some account_move_lines cells can have multiple lines: one for the
                # title then some additional lines for text.
                # On Xlsx it's better to keep everything on one line so when you click
                # on cell, all the value is shown and not juste the title
    ...
    def _write_tax_summaries_to_sheet(  # noqa: C901
    ...
    def _generate_document_data_for_export(self, report, options, export_type="pdf"):
        ...
        :param export_type:     The export type the generation need to use can be ('pdf'
        or 'xslx')

        :return: a dictionnary containing a list of all lines grouped by journals and a
        dictionnay with the global tax summary lines
        - journals_vals (mandatory):                    List of dictionary containing
        all the lines, columns, and tax summaries
            - lines (mandatory):                        A list of dict containing all
            tha data for each lines in format returned by _get_lines_for_journal
            - columns (mandatory):                      A list of columns for this
            journal returned in the format returned by _get_columns_for_journal
            - tax_summary (optional):                   A dict of data for the tax
            summaries inside journals in the format returned by _get_tax_summary_section
        - global_tax_summary:                           A dict with the global tax
        summaries data in the format returned by _get_tax_summary_section
        """
        ...
        query = SQL(
            ...
            SELECT
                ...
                CASE WHEN j.type = 'sale' THEN am.payment_reference WHEN j.type =
                'purchase' THEN am.ref END AS reference,
            ...
            JOIN account_journal j ON j.id = am.journal_id
            JOIN res_company cp ON cp.id = am.company_id
            LEFT JOIN account_move_line_account_tax_rel aml_at_rel ON
            aml_at_rel.account_move_line_id = account_move_line.id
            LEFT JOIN account_tax parent_tax ON parent_tax.id =
            aml_at_rel.account_tax_id and parent_tax.amount_type = 'group'
            LEFT JOIN account_tax_filiation_rel tax_filiation_rel ON
            tax_filiation_rel.parent_tax = parent_tax.id
            LEFT JOIN account_tax tax ON (tax.id = aml_at_rel.account_tax_id and
            tax.amount_type != 'group') or tax.id = tax_filiation_rel.child_tax
            LEFT JOIN account_account_tag_account_move_line_rel tag_rel ON
            tag_rel.account_move_line_id = account_move_line.id
            ...
            WHERE %(case_statement)s AND %(search_conditions)s
            GROUP BY "account_move_line".id, am.id, p.id, %(account_alias)s.id, j.id,
            cp.id, journal_curr.id, account_code, account_name
            ORDER BY
                CASE j.type
                    WHEN 'sale' THEN 1
    ...
    def _get_columns_for_journal(self, journal, export_type="pdf"):
        ...
        :return: A list of the columns as dict each having:
            ...
            - class (optional):     A string with css classes that need to be applied to
            all that column
        """
        columns = [
        ...
        # We have different columns regarding we are exporting to a PDF file or an XLSX
        # document
        if export_type == "pdf":
    ...
    def _get_export_lines_for_journal(
        ...
        """
        Default document lines generation it will generate a list of lines in a format
        valid for the pdf and xlsx

        If it is a bank journal it will be redirected to _get_lines_for_bank_journal
        since this type of journals
        ...
        :return: A list of lines. Each line is a dict having:
            - 'column_label':           A dict containing the values for a cell with a
            key that links to the label of a column
                - data (mandatory):     The formatted cell value
                - class (optional):     Additional css classes to apply to the current
                cell
            - line_class (optional):    Additional css classes that applies to the
            entire line
        """
        lines = []
        ...
        # Add an empty line to add a separation between the total section and the data
        # section
        lines.append({})
    ...
    def _get_export_lines_for_bank_journal(
        ...
        """
        Bank journals are more complex and should be calculated separately from other
        journal types
        ...
        for i, account_move_line_vals_list in enumerate(account_moves_vals_list):
            ...
            for j, move_line_entry_vals in enumerate(account_move_line_vals_list):
                ...
                if move_line_entry_vals["account_type"] not in (
                    ...
                    document = ""
                    if j == 0:
                        document = f"{move_line_entry_vals['move_name']} ({move_line_entry_vals['date']})"  # noqa: E501
                    line = self._get_base_line(
                        report,
                        options,
                        export_type,
    ...
    def _get_base_line(
        ...
        """
        Returns the generic part of a line that is used by both '_get_lines_for_journal'
        and '_get_lines_for_bank_journal'

        :return:                                    A dict with base values for the line
            - line_class (mandatory):                   Css classes that applies to this
            whole line
            - document (mandatory):                     A dict containing the cell data
            for the column document
                - data (mandatory):                         The value of the cell
                formatted
                - class (mandatory):                        css class for this cell
            - account (mandatory):                      A dict containing the cell data
            for the column account
                - data (mandatory):                         The value of the cell
                formatted
            - account_code (mandatory):                 A dict containing the cell data
            for the column account_code
                - data (mandatory):                         The value of the cell
                formatted
            - account_label (mandatory):                A dict containing the cell data
            for the column account_label
                - data (mandatory):                         The value of the cell
                formatted
            - name (mandatory):                         A dict containing the cell data
            for the column name
                - data (mandatory):                         The value of the cell
                formatted
            - debit (mandatory):                        A dict containing the cell data
            for the column debit
                - data (mandatory):                         The value of the cell
                formatted
                - class (mandatory):                        css class for this cell
            - credit (mandatory):                       A dict containing the cell data
            for the column credit
                - data (mandatory):                         The value of the cell
                formatted
                - class (mandatory):                        css class for this cell

            - taxes(optional):                          A dict containing the cell data
            for the column taxes
                - data (mandatory):                         The value of the cell
                formatted
            - tax_grids(optional):                          A dict containing the cell
            data for the column taxes
                - data (mandatory):                         The value of the cell
                formatted
        """
    ...
    def _query_bank_journal_initial_balance(self, options, journal_id):
        ...
        query = SQL(
            """
                ...
                FROM %(table)s
                JOIN account_journal journal ON journal.id =
                "account_move_line".journal_id AND account_move_line.account_id =
    ...
    def _get_generic_tax_report_options(self, options, data):
        """
        Return an option dictionnary set to fetch the reports with the parameters needed
        for this journal.
        The important bits are the journals, date, and fetch the generic tax reports
        that contains all taxes.
        We also provide the information about wether to take all entries or only posted
        ones.
        """
        generic_tax_report = self.env.ref("account.generic_tax_report")
        previous_option = options.copy()
        # Force the dates to the selected ones. Allows to get it correctly when grouped
        # by months
        ...
        # Even though it doesn't have a journal selector, we can force a journal in the
        # options to only get the lines for a specific journal.
    ...
    def _get_tax_grids_summary(self, options, data):
        ...
        The result is grouped by the country in which the tag exists in case of multivat
        environment.
        ...
        """
        report = self.env.ref("l10n_ve_reports.journal_report")
        # Use the same option as we use to get the tax details, but this time to
        # generate the query used to fetch the
        # grid information
        ...
        query = SQL(
            ...
            WITH tag_info (country_name, tag_id, tag_name, tag_sign, balance) AS (
                SELECT
                    ...
                    SUM(COALESCE("account_move_line".balance, 0)
                        * CASE WHEN "account_move_line".tax_tag_invert THEN -1 ELSE 1
                        END
                        ) AS balance
                FROM account_account_tag tag
                JOIN account_account_tag_account_move_line_rel rel ON tag.id =
            ...
            SELECT
                country_name,
                tag_id,
                REGEXP_REPLACE(tag_name, '^[+-]', '') AS name, -- Remove the sign from
                the grid name
                balance,
                tag_sign AS sign
            FROM tag_info
        ...
        for country_name, tag_id, name, balance, sign in query_res:
            ...
            # We need them formatted, to ensure they are displayed correctly in the
            # report. (E.g. 0.0, not 0)
    ...
    def _get_generic_tax_summary_for_sections(self, options, data):
        ...
        Works by forcing specific options into the tax report to only get the lines we
        need.
        The result is grouped by the country in which the tag exists in case of multivat
        environment.
        Returns a dictionary with the following structure:
        {
            Country : [
                {name, base_amount, tax_amount, tax_non_deductible{_no_format},
                tax_deductible{_no_format}, tax_due{_no_format}},
                {name, base_amount, tax_amount, tax_non_deductible{_no_format},
                tax_deductible{_no_format}, tax_due{_no_format}},
                {name, base_amount, tax_amount, tax_non_deductible{_no_format},
                tax_deductible{_no_format}, tax_due{_no_format}},
        ...
        # Make the final data dict that will be used by the template, using the taxes
        # information.
    ...
    def journal_report_tax_tag_template_open_aml(self, options, params=None):
        """returns an action to open a list view of the account.move.line having the selected tax tag"""  # noqa: E501
    ...
    def journal_report_action_open_tax_journal_items(self, options, params):
        ...
        :param params: a dict containing the line params. (Dates, name, journal_id,
        tax_type)
    ...
    def journal_report_action_open_account_move_lines_by_account(self, options, params):
        ...
        :param params: The params given from the report UI (journal_id, account_id,
        date)
        :return: act_window on journal items filtered on the current journal and the
        current account within a date.
        """
        report = self.env["account.report"].browse(options["report_id"])
