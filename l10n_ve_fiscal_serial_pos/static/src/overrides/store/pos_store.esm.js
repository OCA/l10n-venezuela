...
async function loadFiscalMachinesIfNeeded(connection) {
    if (!connection.state.machines?.length && connection.loadSystrayData) {
        await connection.loadSystrayData();
    }
}

async function reconnectIfMachineChanged(connection, previousId, ok) {
...
async function syncFiscalMachineFromPosOrder(pos, env) {
    ...
    await loadFiscalMachinesIfNeeded(connection);
    ...
    return reconnectIfMachineChanged(connection, previousId, ok);
}

patch(PosStore.prototype, {
    async selectInvoiceJournal(...args) {
        const selectedJournal = await super.selectInvoiceJournal(...args);
        if (selectedJournal) {
