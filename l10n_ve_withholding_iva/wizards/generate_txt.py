...
class GenerateTxtWhIva(models.TransientModel):
    ...
    def action_generate_txt(self):
        ...
        company_vat = (company.partner_id.vat or "").replace("VE", "").replace("-", "").strip()
        
        lines = []
        for v in vouchers:
            partner_vat = (v.partner_id.vat or "").replace("VE", "").replace("-", "").strip()
            for l in v.line_ids:
                inv = l.move_id
                ...
                base_amt = f"{l.base_amount:.2f}"
                ret_amt = f"{l.amount_ret:.2f}"
                exempt_amt = "0.00"
                tax_rate = f"{(l.tax_amount / l.base_amount * 100):.2f}" if l.base_amount else "16.00"
                voucher_num = v.name or ""
                
                # SENIAT TXT format:
                # CompanyRIF	Period(YYYYMM)	DocDate(YYYY-MM-DD)	Type(C/V)	DocType(01/02/03)	PartnerRIF	DocNumber	CtrlNumber	TotalAmt	BaseAmt	RetAmt	0	VoucherNum	ExemptAmt	TaxRate	0
                doc_type = "01" if inv.move_type == "in_invoice" else "03"
                txt_row = f"{company_vat}	{v.date.strftime('%Y%m')}	{inv.invoice_date.strftime('%Y-%m-%d')}	C	{doc_type}	{partner_vat}	{inv_number}	{nro_ctrl}	{total_inv}	{base_amt}	{ret_amt}	0	{voucher_num}	{exempt_amt}	{tax_rate}	0"
